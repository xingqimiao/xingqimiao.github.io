...
export default function ReportListPage() {
  ...
  const reports = compiledArticles.filter((art) => art.type === "report");
  const filteredReports = reports.filter((art) =>
        ...
        {filteredReports.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredReports.map((post) => (
              <Link key={post.slug} href={`/report/${post.slug}`} className="fade-in flex">
                <BentoCard 
                  theme="light" 
