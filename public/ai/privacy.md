...
Kira Tracker（[hrt.kiramyao.com](https://hrt.kiramyao.com)）是另一项独立服务，用于记录激素替代治疗（HRT）的用药与血液化验结果，并据此估算激素水平随时间的变化。你可以用账号名与密码登录，也可以用 X 或 Google 登录。它处理的是健康数据，因此单独用本节说明。本节与其前各节如有不一致之处，以本节为准。

### 26.1 加密范围

Kira Tracker 中，以下内容会被合并成一份数据包，以 **AES-256-GCM** 整包加密后才写入数据库（密文形态为 iv:tag:ciphertext，每条记录使用独立的随机初始向量）：
...
* 账号名；
* 账户创建时间与最后更新时间；
* 每一条用药记录、化验结果、备注和设置的时间。数据库需要有明文时间，才能在不逐条解密的情况下排序和分页时间线，因此这些时间可以被读取；
* 每一条记录的**类别**（用药、化验、备注或设置），用于在列表中分类显示；
* 登录与注册尝试所使用的 IP 地址；
* 使用 X 或 Google 登录时关联的服务商身份：X 另外包含用户名、显示名称和头像地址；
* 账户是否存在，以及各类计数。

因此，数据库快照可以显示出你何时、多久记录一次，以及记录的是哪一类，但看不出你填写的具体内容；如果只有数据库文件而没有数据库之外的密钥，密文部分无法读出。

### 26.2 加密的边界

Kira Tracker 不是端到端加密的服务。你的浏览器与服务器之间使用 HTTPS 传输，记录在服务器上静态加密，密钥保存在数据库之外。

**服务器持有的密钥可以打开这些记录，并在把记录返回给你的应用时解密。** 加密真正保护的是「数据库文件被单独带走或备份泄露」这种情况：没有数据库之外的密钥，密文读不出内容。它不是「我们自己看不到你的数据」的承诺，本节也不使用「只有你能看到」这类说法。

你在应用里登录后，服务器会在内存中保留一份解密密钥，空闲超时或主动退出后丢弃。这只影响该登录会话本身，不改变上面那句：服务在运行时，能够解密它被要求返回的记录。
...
* Kira Tracker 不使用统计分析脚本、追踪代码或第三方广告。除注册时的人机验证（见下）之外，该应用不会向任何第三方主机发起请求。
...
* **Google**：仅在你主动使用时。我们只申请 openid 这一个范围，也只读取其中标识你账户的 sub 字段用于识别身份，不会请求或接收你的邮箱、姓名、头像或其他 Google 数据；收到的标识只用于登录识别，不用于广告、用户画像或出售。
* **Oracle Cloud**：服务器位于新加坡区域，数据库存放在此，其中你的记录以密文形式存在。
* **Cloudflare**：负责域名解析、流量转发与安全防护。Kira Tracker 的网页界面由我们自己的服务器提供，Cloudflare 位于其前端。
* **Cloudflare Turnstile**：仅用于注册账号时的人机验证，用来阻止自动化批量注册。它不设置用于广告或跨站追踪的 Cookie，也不参与你的记录内容。使用时浏览器会向 challenges.cloudflare.com 发起请求；除此之外 Kira Tracker 不加载任何第三方脚本。
* **AI 服务提供方**：见 26.3。
* 除上述之外，我们不向其他人共享你的数据。
...
删除单条记录是真正的删除：该行会从数据库中移除，没有软删除或墓碑，删除后无法恢复。

账户删除是即时且完整的，可以在应用内自助完成，删除时需要再次输入密码。记录、化验结果、备注、设置、分享链接、API 令牌以及 X 与 Google 的关联信息都会被移除；含 IP 地址的登录事件记录会被单独删除，已登录的会话也会被撤销。删除后会保留一条不含任何标识信息的计数记录，内容为一个原因说明、账户创建时间和删除时间，用于回答「有多少人自行删除」这类问题，无法据此识别到个人。
