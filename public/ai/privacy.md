...
因此，数据库快照可以显示出你何时、多久记录一次、记录的是哪一类，以及上一条列出的设置（含体重与 HRT 开始日期）；用药、化验、备注与体感记录的**具体内容**仍然读不出，因为它们只有密文。如果只有数据库文件而没有数据库之外的密钥，密文部分无法读出。
...
* 体感记录，你自行填写的文字；
* IP 地址，用于防滥用与频率限制；
* 化验单照片（仅在你使用识别功能时）。识别在**你的设备上**完成，照片不会被上传或保存，只有识别出的数值会变成化验记录。
