...
* **Kira Tracker（HRT 记录工具）及其账户、用药与化验数据（见第 26 节）。**

通过外部链接访问的其他网站、平台或服务，适用其各自的隐私政策和服务规则，不由本说明直接控制。

如果你使用 Kira Tracker，请**优先阅读第 26 节**；该部分与其前面的内容如有冲突，以第 26 节为准。
...
你可以在评论区自行删除自己发表的评论。如需查询、修改或删除更多评论内容，或希望注销账号关联信息，可以通过 [report@kiramyao.com](mailto:report@kiramyao.com) 联系我们。
...
1. 为完成问卷托管、网站运行或文件存储而使用必要的第三方平台（**评论数据不在此列**：它只存储在我们自己的服务器上，不会委托给第三方平台托管）；
...
## 26. Kira Tracker 与 HRT 记录

第 1 至 25 节说明 KiraMyao Equal 的网站、问卷、投稿、故事和评论服务。

Kira Tracker（[hrt.kiramyao.com](https://hrt.kiramyao.com)）是另一项独立服务，用于记录激素替代治疗（HRT）的用药与血液化验结果，并据此估算激素水平随时间的变化。它处理的是健康数据，因此单独用本节说明。本节与其前各节如有不一致之处，以本节为准。

### 26.1 加密范围

Kira Tracker 中，以下内容加密保存，我们无法读取：

* 用药记录，包括给药途径、酯类、剂量及各途径的附加字段；
* 化验结果的数值与单位；
* 体重、HRT 模式、校准设置和药代参数覆盖值。

这些内容以 AES-GCM 加密。每个账户有一把独立的数据密钥，该密钥本身也只以密文形式保存，并使用由你的密码派生出的密钥包裹（PBKDF2，600,000 次迭代，SHA-256）。我们不保存你的密码，只保存它的加盐 scrypt 哈希，而哈希无法推导出密钥。

以下内容不加密，我们可以读取：

* 用户名；
* 账户创建时间与最后更新时间；
* 每一条用药记录和每一条化验结果的时间。数据库需要有明文时间，才能在不逐条解密的情况下排序和分页时间线，因此这些时间可以被读取；
* 登录与注册尝试所使用的 IP 地址；
* 是否启用双重验证，以及启用时间；
* 使用 X 登录时关联的 X 用户名与数字用户 ID；
* 账户是否存在，以及各类计数。

因此，数据库快照可以显示出你何时、多久记录一次，但看不出记录的内容。

### 26.2 加密的边界

Kira Tracker 不是端到端加密的服务。你的浏览器与服务器之间使用 HTTPS 传输，记录在服务器上静态加密；登录之后，服务器会在内存中持有解密密钥，最长为你连续三十分钟没有操作。超时或你主动退出后，该密钥即被丢弃，记录重新变为服务器无法读取。

在会话有效期内，服务器能够读取你打开的记录。本节不使用「只有你能看到」这类说法。
...
* 用户名，用于标识你的账户；
* 密码，用于登录，仅以加盐哈希保存；
* 双重验证密钥，用于校验验证器显示的验证码，单独封存；
* 恢复码，用于在验证器丢失时代替验证码，仅保存哈希；
* 用药记录，这是服务的核心功能；
* 化验结果，用于计算水平估算并按你的数据校准；
* 体重，估算模型需要它计算分布容积；
* IP 地址，用于防滥用与频率限制；
* X 用户 ID 与用户名，仅在你使用 X 登录时用于识别身份。

哪些内容加密、哪些不加密，见 26.1。

另外，以下情况同样适用：

* Kira Tracker 不使用统计分析脚本、追踪代码或第三方广告。经实际请求核实，该应用不会向任何第三方主机发起请求。
* 使用 Kira Tracker 不需要邮箱地址，也不需要真实姓名。
...
* **Cloudflare**：负责域名解析、流量转发与安全防护。Kira Tracker 的网页界面由我们自己的服务器提供，Cloudflare 位于其前端。
* **AI 服务提供方**：见 26.3。
* 除上述之外，我们不向其他人共享你的数据。
...
单条记录的删除是软删除。该条记录会被标记为已删除并从应用中排除，但数据行会保留到你删除账户为止。我们不把这一步表述为彻底移除。

账户删除是即时且完整的，可以在应用内自助完成。记录、化验结果、设置、恢复码、API 令牌和 X 关联信息都会被移除，其中仍含 IP 地址的登录事件记录会被单独删除，所有已解锁的会话也会被撤销。删除后会保留一条不含任何标识信息的计数记录，内容为一个原因说明、账户创建时间和删除时间，用于回答「有多少人自行删除」这类问题，无法据此识别到个人。

如果数据库备份中仍包含已删除的内容，这些副本最长保存三十日，随后随备份轮换而消失。

关于密码和恢复码，有两点需要区分清楚：

* **丢失密码会使记录无法恢复。** 记录的解密密钥由你的密码派生，我们不持有该密钥，也没有重置密码的途径。出于同样的原因，修改密码需要提供当前密码。恢复码不能替代密码：服务器只保存恢复码的哈希，它不携带任何密钥，无法解开记录。
* **丢失验证器和恢复码会让你无法登录。** 登录只接受验证器验证码或一组恢复码，没有其他方式通过双重验证；我们无法为你重置第二因素。这种情况下账户和记录本身仍然存在，如果你之后找到了保存的恢复码，仍可重新登录。它影响的是访问，不是记录本身。

### 26.7 你的权利

* 查阅与导出：应用内提供 CSV、PDF、JSON 导出。
* 更正：你可以直接在应用内修改记录。
...
Kira Tracker 不面向 13 岁以下儿童，我们也不会在知情情况下为其保留账户。如你认为有 13 岁以下儿童注册了账户，可以写信至 **report@kiramyao.com**，我们会删除该账户。已满 13 岁但未达到所在地成年年龄的使用者，应取得父母或监护人的同意。

未成年使用者的记录同样处于加密状态、我们无法读取，但我们可以删除账户，且删除会连带清除记录。
...
* 密码仅以加盐 scrypt 哈希保存；
* 所有账户强制启用双重验证；
* 双重验证密钥在数据库之外的密钥下加密封存；
* 登录具备频率限制与账户锁定；
* 修改密码时删除既有的 API 令牌。

我们不声称持有任何我们并未取得的认证。Kira Tracker 不是医疗器械，也不声称符合 HIPAA 等医疗合规标准。
